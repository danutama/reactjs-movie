...
const AllTVAiringToday = () => {
  ...
  const storageKeyPrefix = 'airingTodayTV';
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const [genreData, tvData] = await Promise.all([fetchTVGenres(), fetchTVAiringToday(page)]);
  ...
  const loadMoreTVShows = async () => {
    ...
    try {
      const tvData = await fetchTVAiringToday(nextPage);
