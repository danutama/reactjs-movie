...
import { fetchPopularTVShows, fetchTVGenres } from '../../service/api';
...
function TvPopular() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      const [genreData, tvData] = await Promise.all([fetchTVGenres(), fetchPopularTVShows()]);
      setGenres(genreData);
      setTvShows(tvData);
      setLoading(false);
    };
  ...
  return (
    <Container>
      <div className="d-flex justify-content-between align-items-baseline mb-3">
        <p className="h5 text m-0">Popular TV Shows</p>
        <Link to="/popular-tv-shows" className="btn-link">
          <small>Explore</small>
        </Link>
      </div>
      {loading ? (
