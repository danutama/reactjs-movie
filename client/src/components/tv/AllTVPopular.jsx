...
const AllTVPopular = () => {
  ...
  const storageKeyPrefix = 'popularTV';
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const [genreData, tvData] = await Promise.all([fetchTVGenres(), fetchPopularTVShows(page)]);
  ...
  const loadMoreTVShows = async () => {
    ...
    try {
      const tvData = await fetchPopularTVShows(nextPage);
