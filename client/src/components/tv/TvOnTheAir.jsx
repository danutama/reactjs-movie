...
function TvOnTheAir() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      const [genreData, tvData] = await Promise.all([fetchTVGenres(), fetchTVOnTheAir()]);
      setGenres(genreData);
      setTvShows(tvData);
      setLoading(false);
    };
  ...
  return (
    <Container>
      <div className="d-flex justify-content-between align-items-baseline mb-3">
        <p className="h5 text m-0">TV Shows On The Air</p>
        <Link to="/tv-shows/on-the-air" className="btn-link">
          <small>Explore</small>
        </Link>
      </div>
      {loading ? (
