...
import { fetchTrendingTVShows, fetchTVGenres } from '../../service/api';
...
function TvTrending() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      const [genreData, tvData] = await Promise.all([fetchTVGenres(), fetchTrendingTVShows()]);
      setGenres(genreData);
      setTvShows(tvData);
      setLoading(false);
    };
  ...
  return (
    <Container>
      <div className="d-flex justify-content-between align-items-baseline mb-3">
        <p className="h5 text m-0">Trending TV Shows</p>
        <Link to="/tv-shows/trending" className="btn-link">
          <small>Explore</small>
        </Link>
      </div>
      {loading ? (
