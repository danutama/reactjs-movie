...
import { fetchTvShowById, fetchTVTrailer } from '../../service/api';
...
function SingleTvShow() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        // fetchTvShowById sudah include credits & external_ids dari BE
        // fetchTVTrailer jalan paralel
        const [tvShowData, trailers] = await Promise.all([fetchTvShowById(id), fetchTVTrailer(id)]);
        ...
        // Credits sudah ada di tvShowData.credits dari BE
        if (tvShowData.credits) {
          setCredits([...(tvShowData.credits.cast || []), ...(tvShowData.credits.crew || [])]);
        }

        // External IDs sudah ada di tvShowData.external_ids dari BE
        if (tvShowData.external_ids) {
          setExternalIds(tvShowData.external_ids);
        }
  ...
  const hasImdbId = Boolean(externalIds?.imdb_id);
  const hasTvGenre = tvShow.genres?.length > 0;
  const hasStatus = Boolean(tvShow.status);
  const hasLanguage = tvShow.spoken_languages?.length > 0;
